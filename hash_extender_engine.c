...
static void md4_hash(uint8_t *data, uint64_t length, uint8_t *buffer,
                     uint8_t *state, uint64_t state_size);
static void md5_hash(uint8_t *data, uint64_t length, uint8_t *buffer,
                     uint8_t *state, uint64_t state_size);
static void ripemd160_hash(uint8_t *data, uint64_t length, uint8_t *buffer,
                           uint8_t *state, uint64_t state_size);
static void sha_hash(uint8_t *data, uint64_t length, uint8_t *buffer,
                     uint8_t *state, uint64_t state_size);
static void sha1_hash(uint8_t *data, uint64_t length, uint8_t *buffer,
                      uint8_t *state, uint64_t state_size);
static void sha256_hash(uint8_t *data, uint64_t length, uint8_t *buffer,
                        uint8_t *state, uint64_t state_size);
static void sha512_hash(uint8_t *data, uint64_t length, uint8_t *buffer,
                        uint8_t *state, uint64_t state_size);
static void tiger192v1_hash(uint8_t *data, uint64_t length, uint8_t *buffer,
                            uint8_t *state, uint64_t state_size);
static void tiger192v2_hash(uint8_t *data, uint64_t length, uint8_t *buffer,
                            uint8_t *state, uint64_t state_size);
#ifndef DISABLE_WHIRLPOOL
static void whirlpool_hash(uint8_t *data, uint64_t length, uint8_t *buffer,
                           uint8_t *state, uint64_t state_size);
#endif

/* The hashing function that each implementation needs to impelement. */
typedef void(hash_t)(uint8_t *data, uint64_t length, uint8_t *buffer,
                     uint8_t *state, uint64_t state_size);
...
char *hash_type_array[] = {"md4",       "md5",        "ripemd160",
                           "sha",       "sha1",       "sha256",
                           "sha512",    "tiger192v1", "tiger192v2",
#ifndef DISABLE_WHIRLPOOL
                           "whirlpool",
#endif
                           NULL};
...
uint8_t *hash_append_data(char *hash_type_name, uint8_t *data,
                          uint64_t data_length, uint64_t secret_length,
                          uint8_t *append, uint64_t append_length,
                          uint64_t *new_length) {
  hash_type_t *hash_type = get_hash_type(hash_type_name);
  /* Allocate memory for the new buffer (enough room for buffer + two full block
   * (finish the current block, entire next block) + the data) */
  /* Note that this can overflow, so this can't be used in security-sensitive
   * applications! */
  uint8_t *result =
      malloc(data_length + append_length + (2 * hash_type->block_size));
  uint64_t bit_length;
  ...
  *new_length = data_length;
  ...
  while (((*new_length + secret_length) % hash_type->block_size) !=
         (hash_type->block_size - hash_type->length_size))
    result[(*new_length)++] = 0x00;

  /* Convert the original length to bits so we can append it. */
  bit_length = (secret_length + data_length) * 8;

  /* Get to within exactly 8 bytes of the end (since we only store 64-bits of
   * length). If we ever need sizes bigger than 64 bits, this needs to change.
   */
  while (((*new_length + secret_length) % hash_type->block_size) !=
         (hash_type->block_size - 8))
    result[(*new_length)++] = 0x00;
...
void hash_gen_signature(char *hash_type_name, uint8_t *secret,
                        uint64_t secret_length, uint8_t *data,
                        uint64_t data_length, uint8_t *signature) {
  uint8_t *buffer;
...
void hash_gen_signature_evil(char *hash_type_name, uint64_t secret_length,
                             uint64_t data_length, uint8_t original_signature[],
                             uint8_t *append, uint64_t append_length,
  ...
  original_data_length =
      (((secret_length + data_length + hash_type->length_size) /
        hash_type->block_size) *
       hash_type->block_size) +
      hash_type->block_size;
  hash_type->hash(append, append_length, new_signature, original_signature,
                  original_data_length);
}

static int hash_test_validate(char *hash_type_name, uint8_t *secret,
                              uint64_t secret_length, uint8_t *data,
                              uint64_t data_length, uint8_t *signature) {
  hash_type_t *hash_type = get_hash_type(hash_type_name);
  ...
  hash_gen_signature(hash_type_name, secret, secret_length, data, data_length,
                     result);
...
static void hash_test_extension(char *hash_type_name) {
  ...
  hash_gen_signature(hash_type_name, secret, strlen((char *)secret), data,
                     strlen((char *)data), original_signature);

  /* Create the new data. */
  new_data = hash_append_data(hash_type_name, data, strlen((char *)data),
                              strlen((char *)secret), append,
                              strlen((char *)append), &new_length);

  /* Generate an evil signature with the data appended. */
  hash_gen_signature_evil(hash_type_name, strlen((char *)secret),
                          strlen((char *)data), original_signature, append,
                          strlen((char *)append), new_signature);

  /* Check the new signature. */
  test_check_boolean(" basic extension",
                     hash_test_validate(hash_type_name, secret,
                                        strlen((char *)secret), new_data,
                                        new_length, new_signature));
...
static void hash_test_lengths(char *hash_type_name) {
  ...
  for (i = 0; i < 3; i++) {
    for (j = 0; j < 993; j++) {
      ...
      /* Get the original signature. */
      hash_gen_signature(hash_type_name, secret, s_len, data, d_len,
                         original_signature);

      /* Create the new data. */
      new_data = hash_append_data(hash_type_name, data, d_len, s_len, append,
                                  a_len, &new_length);

      /* Generate an evil signature with the data appended. */
      hash_gen_signature_evil(hash_type_name, s_len, d_len, original_signature,
                              append, a_len, new_signature);

      /* Check the new signature. */
      test_check_boolean(text, hash_test_validate(hash_type_name, secret, s_len,
                                                  new_data, new_length,
                                                  new_signature));
