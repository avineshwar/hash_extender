...
static void test_failed(void) { tests_run++; }
...
void test_check_memory(char *description, uint8_t *expected,
                       uint64_t expected_length, uint8_t *result,
                       uint64_t result_length) {
  if (expected_length == result_length &&
      !memcmp(expected, result, expected_length)) {
    test_passed();
  } else {
    test_failed();
...
void test_report(void) {
  if (tests_run == 0) {
    ...
    printf("TESTS PASSED: %d / %d [%2.4f%%]\n", tests_passed, tests_run,
           100 * (float)tests_passed / tests_run);
    printf("-------------------------------------------------------------------"
           "-------------\n");
  }
