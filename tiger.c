...
#define T1(c) (t1[(c>>0)&0xff])
#define T2(c) (t2[(c>>16)&0xff])
#define T3(c) (t3[(c>>32)&0xff])
#define T4(c) (t4[(c>>48)&0xff])

#define T1L(c) (t4[(c>>8)&0xff])
#define T2L(c) (t3[(c>>24)&0xff])
#define T3L(c) (t2[(c>>40)&0xff])
#define T4L(c) (t1[(c>>56)&0xff])

#define C1(c) (T1(c) ^ T2(c) ^ T3(c) ^ T4(c))
#define C2(c) (T1L(c) ^ T2L(c) ^ T3L(c) ^ T4L(c))
...
static void pass(uint64_t *aa, uint64_t *bb, uint64_t *cc, uint64_t *x, uint8_t mul) {
    uint64_t a = *aa, b = *bb, c = *cc;
    ROUND(a, b, c, x[0], mul);
    ROUND(b, c, a, x[1], mul);
    ROUND(c, a, b, x[2], mul);
    ROUND(a, b, c, x[3], mul);
...
int TIGER_Init(TIGER_CTX *hasher)
{
...
{
    uint64_t a = hasher->state[0], b = hasher->state[1],c = hasher->state[2];
    uint64_t *x;
    x = malloc(TIGER_BLOCK_SIZE);
    memcpy((uint8_t*) x, data, TIGER_BLOCK_SIZE);
...
int TIGER_Update(TIGER_CTX *hasher, const void *data, size_t len)
{
    ...
    if (len >= part_len) {
        memcpy(hasher->buffer + index, data, part_len);
        TIGER_Transform(hasher, hasher->buffer);
        for (i = part_len; i + 63 < len; i += 64) {
            TIGER_Transform(hasher, data + i);
        }
        index = 0;
    } else {
        i = 0;
    ...
    memcpy(hasher->buffer + index, data + i, len - i);
    return 1;
}

int TIGER_Final(unsigned char *md, TIGER_CTX *hasher) {
    uint32_t index;
    uint8_t i, *padding;
    padding = malloc(TIGER_BLOCK_SIZE);
    ...
    for (i = 0; i < 8; i++) {
        ...
        md[i+8] = (uint8_t) (hasher->state[1] >> (8 * i));
        md[i+16] = (uint8_t) (hasher->state[2] >> (8 * i));
    }
    free(padding);
    return 1;
}